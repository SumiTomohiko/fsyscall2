...
def make_enum_command(codes):
    return (",\n" + make_indent(4)).join([code.name for code in codes])

def make_command_list(codes, fmt):
    sep = ";\n" + make_indent(8)
    return sep.join([fmt.format(**vars(code)) for code in codes])

def make_number2command(codes):
    fmt = "mNumber2Command.put(Integer.valueOf({value}), {name})"
    return make_command_list(codes, fmt)

def make_command2number(codes):
    fmt = "mCommand2Number.put({name}, Integer.valueOf({value}))"
    return make_command_list(codes, fmt)
